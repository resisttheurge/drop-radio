import { Group, Path, PathProps, Skia } from '@shopify/react-native-skia'
...
export interface FallingManProps extends PathProps {
  index?: number
  fgColor?: string | DerivedValue<string> | SharedValue<string>
  bgColor?: string | DerivedValue<string> | SharedValue<string>
  scale?: number | DerivedValue<number> | SharedValue<number>
  opacity?: number | DerivedValue<number> | SharedValue<number>
  degrees?: number | DerivedValue<number> | SharedValue<number>
}

const fallingManPath = Skia.Path.MakeFromSVGString(
...
export function SpinningMan({ theme, canvasSize }: VisualizationProps) {
  const { colors } = theme
  ...
  const degrees = useDerivedValue(() => {
    return interpolate(rpm.value, [0, slow, fast], [0, 10, 360])
  })
